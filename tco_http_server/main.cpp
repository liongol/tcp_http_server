...
#define PORT 8080
#define BUFFER_SIZE 1024
#define INACTIVE_TIMEOUT 10 // 120 seconds

enum class Status { IDLE, READING, WRITING, FINISHED };
enum class HttpReq { GET_, POST_, PUT_, DELETE_, HEAD_, TRACE_, OPTIONS_, NOT_SET_YET_, NOT_SUPPORTED_ };
...
int main()
{
    ...
    if (ioctlsocket(listenSocket, FIONBIO, &nonBlocking) == SOCKET_ERROR) {
        std::cout << "Failed to set listen socket as non-blocking: " << WSAGetLastError() << std::endl;
        closesocket(listenSocket);
        WSACleanup();
        return 1;
    }
    ...
    while (true)
    {
        ...
        if (FD_ISSET(listenSocket, &temp)) {
            ...
            if (ioctlsocket(clientSocket, FIONBIO, &nonBlocking) == SOCKET_ERROR) {
                std::cout << "Failed to set client socket as non-blocking: " << WSAGetLastError() << std::endl;
            ...
            SocketState newSocket;
            newSocket.id = clientSocket;
            newSocket.status = Status::IDLE;
            newSocket.reqType = HttpReq::NOT_SET_YET_;
            newSocket.lastAction = time(NULL);
            newSocket.bufferLen = 0;
            newSocket.readBufferLen = 0;
            newSocket.writeBufferLen = 0;
            sockets.push_back(newSocket);

            // Add the new connection to the master FD_SET
            FD_SET(clientSocket, &master);

            std::cout << "New connection accepted with socket ID: " << clientSocket << std::endl;
        }
        ...
        for (int i = 0; i < sockets.size(); i++) {
            ...
            if (FD_ISSET(socket.id, &temp)) {
                ...
                // For now, just support GET requests

                // Check if this is the first time we're reading from this socket
                if (socket.status == Status::IDLE) {
					// Check if this is a GET request
                    if (strncmp(buffer, "GET", 3) == 0) {
						// This is a GET request
						socket.status = Status::READING;
						socket.reqType = HttpReq::GET_;
						socket.readBufferLen = 0;
						socket.writeBufferLen = 0;
						socket.bufferLen = 0;
					}
                    else {
						// This is not a GET request
						std::cout << "Unsupported request type." << std::endl;
						closesocket(socket.id);
						FD_CLR(socket.id, &master);
						sockets.erase(sockets.begin() + i);
						continue;
                ...
                if (socket.status == Status::READING) {
					// Append the new data to the read buffer
					memcpy(socket.readBuffer + socket.readBufferLen, buffer, bytesReceived);
					socket.readBufferLen += bytesReceived;

					// Check if we've reached the end of the HTTP request
                    if (socket.readBufferLen >= 4) {
						// Check if the last 4 bytes are \r\n\r\n
                        if (strncmp(socket.readBuffer + socket.readBufferLen - 4, "\r\n\r\n", 4) == 0) {
							// We've reached the end of the HTTP request
							socket.status = Status::WRITING;
							socket.bufferLen = 0;
							socket.writeBufferLen = 0;
						}
					}
                ...
                std::string response = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n<html><body><h1>Hello, World!</h1></body></html>";
                // Write to the socket
                if (socket.status == Status::WRITING) {
                    // Copy the response to the write buffer
                    memcpy(socket.writeBuffer, response.c_str(), response.length());
                    socket.writeBufferLen = response.length();

                    // Write the response to the socket
                    int bytesSent = send(socket.id, socket.writeBuffer + socket.bufferLen, socket.writeBufferLen - socket.bufferLen, 0);
					...
					socket.bufferLen += bytesSent;

					// Check if we've finished sending the response
					if (socket.bufferLen >= socket.writeBufferLen) {
                        // We've finished sending the response. If so, do NOT close the socket yet.
                        // We need to wait for the client to close the connection, or until INACTIVE_TIMEOUT is reached.
                        socket.status = Status::IDLE;
                        socket.reqType = HttpReq::NOT_SET_YET_;
                        socket.bufferLen = 0;
                        socket.readBufferLen = 0;
                        socket.writeBufferLen = 0;
                        }
